...
describe('stripAnsi', () => {
  it('strips CSI private-mode (bracketed paste toggle)', () => {
    expect(stripAnsi('hello\x1b[?2004hworld')).toBe('helloworld');
    expect(stripAnsi('a\x1b[?2004l b')).toBe('a b');
  });
  it('strips SGR (colors) and other CSI commands', () => {
  ...
  it('normalizes CRLF and strips bare CR', () => {
  ...
  it('strips backspace/bell/vertical-tab/feed', () => {
    expect(stripAnsi('a\x08b\x07c\x0bd\x0ce')).toBe('abcde');
  });
  it('handles full Ubuntu prompt sequence', () => {
    const raw = '\r\n\x1b[?2004l\r\x1b[?2004hroot@ubuntu2204-arm64:~# ';
    // \r\n → \n, \x1b[?2004l → '', \r → '', \x1b[?2004h → ''
    expect(stripAnsi(raw)).toBe('\nroot@ubuntu2204-arm64:~# ');
  });
});

describe('bytesToTermText', () => {
  it('decodes UTF-8 and strips ANSI together', () => {
    const bytes = new TextEncoder().encode('hello\x1b[?2004h 世界');
    expect(bytesToTermText(bytes)).toBe('hello 世界');
  });
  it('replaces invalid UTF-8 with U+FFFD', () => {
    const bytes = new Uint8Array([0x48, 0xff, 0x69]); // H + 续位? + i
    expect(bytesToTermText(bytes)).toBe('H\u{FFFD}i');
  });
});
