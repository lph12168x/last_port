...
[![Build & Release](https://github.com/lph12168x/last_port/actions/workflows/release.yml/badge.svg)](https://github.com/lph12168x/last_port/actions/workflows/release.yml)
...
- **Windows**: 不打 MSI/NSIS 安装器,只产 portable exe + zip (Release 页面下载)
  - Win11 自带 WebView2 Runtime,解压即用
  - Win10 需先装一次 [WebView2 Evergreen Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)
...
- **Linux**: 用户需要属于 `dialout` 组才能打开 `/dev/ttyUSB0` 等:
  ```bash
  sudo usermod -aG dialout $USER
  # 然后重新登录
  ```
- **macOS**: USB-Serial 通常无需额外配置,某些设备需在系统设置授权
- **Windows**: USB-Serial 设备需要安装对应驱动:
  - CH340 / CH341 → `ch341ser` 驱动
  - CP210x → Silicon Labs VCP 驱动
  - FTDI → FTDI VCP 驱动 (一般 Win10/11 自动识别)
  - PL2303 → Prolific 驱动

  没有驱动时设备不会出现在 COM* 列表中。打开 **设备管理器** → **端口 (COM & LPT)** 确认设备是否被识别。

### 看不到设备?

1. 点击左侧 **↻** 刷新按钮
2. 查看 ConfigPanel 中的提示信息
3. 如列表为空,检查:
   - USB 物理连接 (换线 / 换口)
   - 设备管理器是否识别 (Windows)
   - 驱动是否安装 (CH340/CH341/CP210x 等)
   - `dialout` 组权限 (Linux: `groups | grep dialout`)
   - macOS 系统设置 → 隐私与安全 → 询问是否允许访问新连接的 USB 设备
