...
mod tests {
    ...
    /// 前端 wire format 对齐: data_bits/stop_bits 是数字, parity/flow_control 是 lowercase 字符串.
    /// 一旦 Rust 端枚举序列化形态变更,此测试立即失败 (曾因 DataBits 无 rename_all 而
    /// 导致前端 invoke('open_port') 反序列化失败: "invalid type: integer `8`, expected string or map").
    #[test]
    fn port_config_wire_format_matches_frontend() {
        let cfg = PortConfig {
            data_bits: DataBits::Eight,
            stop_bits: StopBits::One,
            parity: Parity::None,
        ...
        // 数字枚举 + lowercase 字符串枚举
        assert!(json.contains("\"data_bits\":8"), "json: {json}");
        assert!(json.contains("\"stop_bits\":1"), "json: {json}");
        assert!(json.contains("\"parity\":\"none\""), "json: {json}");
        assert!(json.contains("\"flow_control\":\"none\""), "json: {json}");

        // 前端发出的真实 payload 能反序列化
        let payload = r#"{"baud_rate":115200,"data_bits":8,"stop_bits":1,"parity":"none","flow_control":"none","read_timeout_ms":50}"#;
        let parsed: PortConfig = serde_json::from_str(payload).unwrap();
        assert_eq!(parsed.data_bits, DataBits::Eight);
        assert_eq!(parsed.stop_bits, StopBits::One);
    }
}