...
/// 列出系统中所有可见串口
///
/// 日志输出: stderr + 文件双写 (见 `crate::log` 模块)
/// - Windows: %APPDATA%\last_port\last_port.log
/// - macOS:   ~/Library/Logs/last_port/last_port.log
/// - Linux:   ~/.local/share/last_port/last_port.log
pub fn list_ports() -> Result<Vec<PortInfo>> {
    log!("[serial::list_ports] called, serialport crate version = {}", env!("CARGO_PKG_VERSION"));
    match serialport::available_ports() {
        Ok(ports) => {
            log!("[serial::list_ports] OK, {} port(s) found:", ports.len());
            for p in &ports {
                log!(
                    "  - {} ({:?})",
                    p.port_name,
            ...
            // 过滤掉 Linux 内置虚拟 UART (通常是 8250/16550 legacy) — 用户
            // 实际接入设备的真实端口名通常是 ttyUSB*/ttyACM*/ttyXR* 等.这些
            // 端口在设备管理器或 dmesg 里大量出现, 会淹没 UI.
            // Windows / macOS 不过滤 (Windows 的 Unknown 可能是 legacy COM1/COM2).
            let ports: Vec<_> = ports
                .into_iter()
                .filter(|p| !is_noise_port(&p.port_name))
                .collect();
            log!("[serial::list_ports] after noise filter: {} port(s)", ports.len());
            Ok(ports.iter().map(PortInfo::from_spinfo).collect())
        }
        Err(e) => {
            log!("[serial::list_ports] ERR: {:?} ({})", e, e);
            Err(SerialError::Serial(e))
        }
    }
}

/// Linux-only: 过滤掉不需要显示给用户的"噪声"端口
#[cfg(target_os = "linux")]
fn is_noise_port(name: &str) -> bool {
    // 内置 8250/16550 UART (最多 4 个, 通常没有真实设备)
    // VMware / VirtualBox 虚拟 UART 也走这一组
    name.starts_with("/dev/ttyS")
        // kernel printk 虚拟串口
        || name == "/dev/ttyprintk"
        // PTY master/slave (sshd / screen / tmux)
        || name.starts_with("/dev/pts/")
        || name.starts_with("/dev/ptmx")
        // 串行控制台 (如果有)
        || name == "/dev/ttyconsole"
}
...
fn is_noise_port(_name: &str) -> bool {
    false
}
